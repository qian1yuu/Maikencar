...
void Mecanum_Wheel_Control(uint8_t wheel, uint8_t duty_cycle, uint8_t dir)
{
  switch(wheel)
  {
    case 1: // 左上轮：A0(PWM)、E2(正)、E3(反)
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, duty_cycle);
      HAL_GPIO_WritePin(GPIOE, GPIO_PIN_2, dir ? GPIO_PIN_SET : GPIO_PIN_RESET);
      HAL_GPIO_WritePin(GPIOE, GPIO_PIN_3, dir ? GPIO_PIN_RESET : GPIO_PIN_SET);
      break;

    case 2: // 右上轮：A1(PWM)、E4(正)、E5(反)
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, duty_cycle);
      HAL_GPIO_WritePin(GPIOE, GPIO_PIN_4, dir ? GPIO_PIN_SET : GPIO_PIN_RESET);
      HAL_GPIO_WritePin(GPIOE, GPIO_PIN_5, dir ? GPIO_PIN_RESET : GPIO_PIN_SET);
      break;

    case 3: // 左下轮：A2(PWM)、c0(正)、C1(反)
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, duty_cycle);
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_0, dir ? GPIO_PIN_SET : GPIO_PIN_RESET);
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_1, dir ? GPIO_PIN_RESET : GPIO_PIN_SET);
      break;

    case 4: // 右下轮：A3(PWM)、C2(正)、C3(反)
      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_4, duty_cycle);
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_2, dir ? GPIO_PIN_SET : GPIO_PIN_RESET);
      HAL_GPIO_WritePin(GPIOC, GPIO_PIN_3, dir ? GPIO_PIN_RESET : GPIO_PIN_SET);
      break;
  }
}
