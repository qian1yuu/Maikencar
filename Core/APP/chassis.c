...
void Chassis_Task(uint8_t en, float vx, float vy, float vw){
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // 开环直接输出PWM，不用PID，不卡！
        float pwm = fabs(wheel_target_rps[i]) * MAX_PWM / CODE_MAX_SPEED;

        if(pwm > MAX_PWM) pwm = MAX_PWM;
        if(pwm < 15)  pwm = 15;  // 启动死区

        Motor_Control(i, wheel_dir[i], (uint8_t)pwm);
    }
}
